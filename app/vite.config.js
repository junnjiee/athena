...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      // ATHENA — the Singapore data plane lives on the Athena server, not in
      // this file's middlewares: those feeds are quota-governed (6 req/10 s
      // keyless) and cached server-side, and the LTA DataMall key must never
      // reach the browser.
      proxy: {
        '/api/sg': {
          target: env.ATHENA_SERVER_URL || 'http://localhost:8787',
          changeOrigin: true,
        },
      },
